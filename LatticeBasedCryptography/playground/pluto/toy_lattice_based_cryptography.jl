...
# ╔═╡ f503d8eb-0ae3-4e8a-b002-5fac6850b2aa
begin
	using LinearAlgebra: det
	using Symbolics: Num
end

# ╔═╡ 36d0bb54-8dff-11f0-3849-bb1b2d67e1c5
md"""
# Lattice-based cryptography; a toy model

References: 青野, 安田. 格子暗号解読のための数学的基礎.
"""

# ╔═╡ 76bffc94-1c34-483f-9e9d-36cf8f549057
begin
	b1 = [
		-4179163,
		-1882253,
	...
	C = hcat(c1, c2, c3) # secret key
	nothing
end

# ╔═╡ 0298cb42-5e9f-4803-bd5b-daf35aa98a78
begin
	m = [86, -35, -32] # message
	e = [-4, -3, 2] # random vector

	w = B * m + e # encrypt
end

# ╔═╡ 2b09c308-0b9b-4fa1-b14c-d4f19757cb39
begin
	# decrypt
	# 暗号化ベクトル w に最も近い格子ベクトル v を計算する．
	v = C * round.(Int, (inv(C) * w))
	@assert v == B * m 
	@assert round.(Int, inv(B) * v) == m
end

# ╔═╡ ebf1eb37-ebff-4fe9-a28f-d6adbfdcbcfe
begin
	T = round.(Int, inv(C) * B)
	@assert B == C * T
	@assert det(Num.(T)) ∈ [-1, 1] # T ∈ GL₃(ℤ)
end
